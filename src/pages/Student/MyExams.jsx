...
export default function MyExams() {
  ...
  useEffect(() => {
    async function loadExams() {
      ...
      const studentDiv = user?.divisionId || 'DIV001';
      const userExams = (allExams || []).filter(
        (e) => !e.divisionIds || e.divisionIds.length === 0 || e.divisionIds.includes(studentDiv)
      );
      setExams(userExams);
    }
    loadExams();
        ...
        <div className={styles.grid}>
          {filtered.length === 0 ? (
            <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '3rem', color: 'var(--text-muted)' }}>
              No examination papers assigned for your division under this filter.
            </div>
          ) : (
            filtered.map((exam, i) => (
              <motion.div
              ...
              >
                ...
                <div className={styles.metaRow}>
                  <MdCalendarToday /> <span>{formatDate(exam.date)}</span>
                </div>
                <div className={styles.metaRow}>
                  <MdTimer /> <span>{formatMinutes(exam.duration)} · {exam.totalQuestions || 20} questions</span>
                </div>
